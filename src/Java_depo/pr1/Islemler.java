...
// {1001,"un ankara cuval 100 2a"}--> map element
    ...
    private static void rafDegisim() {
        System.out.print("rafını güncelleyeceginiz urun id girniz : ");
        int arananId = scan.nextInt();
        if (urunler.keySet().contains(arananId)) {
            System.out.print("guncel raf bilgisini girniz : ");
            int guncelRaf = scan.nextInt();
            urunler.get(arananId).setRaf(guncelRaf);

            System.out.println("urun rafınız degistirildi ");
        } else System.out.println("aradıgınız urun yok");

    }

    private static void uruncıkıs() {
        System.out.print("miktar cıkısı yapılacak urun id girniz : ");
        int arananId = scan.nextInt();
        if (urunler.keySet().contains(arananId)) {
            System.out.print("cıkıs miktarınızı girniz : ");
